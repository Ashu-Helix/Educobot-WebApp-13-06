...
function create() {
    ...
    if (leftLights[0] == null) {
        leftLights.push(this.add.pointlight(gameWidth * 0.13, topY, 0, radius, intensity));
    ...
    if (leftLights[1] == null) {
        leftLights.push(this.add.pointlight(gameWidth * 0.13, midY, 0, radius, intensity));
    ...
    if (leftLights[2] == null) {
        leftLights.push(this.add.pointlight(gameWidth * 0.13, botY, 0, radius, intensity));
    ...
    if (rightLights[0] == null) {
        rightLights.push(this.add.pointlight(gameWidth * 0.43, topY, 0, radius, intensity));
    ...
    if (rightLights[1] == null) {
        rightLights.push(this.add.pointlight(gameWidth * 0.43, midY, 0, radius, intensity));
    ...
    if (rightLights[2] == null) {
        rightLights.push(this.add.pointlight(gameWidth * 0.43, botY, 0, radius, intensity));
        rightLights[2].color.setTo(42, 255, 24);
    } else {
        this.add.existing(rightLights[2]);
    }

    this.add.image(gameWidth * 0.28, gameHeight * 0.52, GAME_CONSTANT.trafficSignal.name);
    ...
    // add sound effects
    if (bgm == null) {
        bgm = this.sound.add("bgm", { loop: true, volume: 0.03 });
        bgm.play();
    }
...
function evaluate_task() {
    ...
    if (!isStageCompleted) {
        ...
        } else if (player_history.length >= 36 && !hasWrongMessage) {
